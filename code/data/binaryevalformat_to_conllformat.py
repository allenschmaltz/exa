# -*- coding: utf-8 -*-
"""
This takes as input binaryevalformat sentences and labels and re-saves as CONLL tab-separated format.
...
def convert_to_conll(sentences_tokens, token_level_labels):
    ...
    for sentence, labels in zip(sentences_tokens, token_level_labels):
        for token, label in zip(sentence, labels):
            alpha_label = "C" if label == 0 else "U"
            conll_output.append(f"{token}\t{alpha_label}\n")
        conll_output.append("\n")
    return conll_output
