# -*- coding: utf-8 -*-
"""
This takes as input binaryevalformat sentences and labels and re-saves as CONLL tab-separated format, where here
we only save monolabels (i.e., the token-level label is always the same as the sentence-level label). This is just
to check that code is not inadvertently using signal from the token-level.
...
def convert_to_conll(sentences_tokens, token_level_labels):
    conll_output = []
    for sentence, labels in zip(sentences_tokens, token_level_labels):
        mono_alpha_label = "C" if sum(labels) == 0 else "U"
        for token, label in zip(sentence, labels):
            # standard label is: alpha_label = "C" if label == 0 else "U"
...
def main(arguments):
    ...
    assert args.input_binaryevalformat_file.strip() != args.input_binaryevalformat_labels_file
    ...
    conll_output = convert_to_conll(sentences_tokens, token_level_labels)
