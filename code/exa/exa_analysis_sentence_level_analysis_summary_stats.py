"""
Calculate summary stats to evaluate heuristics for uncertainty/prediction reliability for admitting
sentence-level predictions. This only makes use of sentence-level labels and would typically be run on the
held-out dev. The resulting constraints can then be used with exa_analysis_sentence_level_analysis.py.
...
def score_output(prediction_stats, options):
    ...
    summary_stats_among_admitted = {"total_admitted_correct": [],
                                    "total_admitted_wrong": [],
                                    "admitted_proportion_correct": [],
                                    "admitted_proportion_wrong": []}
    ...
    for archive_index in range(len(prediction_stats)):
        ...
        if np.sum(admit_constraints) > 0:
            ...
            if admitted_sentence_level_prediction != true_sentence_level_label:
                # print(f"Incorrect prediction sentence index: {sent_i}, "
                #       f"true label: {true_sentence_level_label}")
                summary_stats_among_admitted["total_admitted_wrong"].append(np.sum(admit_constraints))
                summary_stats_among_admitted["admitted_proportion_wrong"].append(np.mean(admit_constraints))
            else:
                summary_stats_among_admitted["total_admitted_correct"].append(np.sum(admit_constraints))
                summary_stats_among_admitted["admitted_proportion_correct"].append(np.mean(admit_constraints))
    ...
    # constrained:
    print("-------Note the following are restricted to sentences with at least 1 admitted token-------")
    ...
    print_stats("Among tokens in correct sentences, total admitted tokens",
                summary_stats_among_admitted["total_admitted_correct"])
    print_stats("Among tokens in correct sentences, proportion admitted",
                summary_stats_among_admitted["admitted_proportion_correct"])
    print_stats("Among tokens in wrong sentences, total admitted tokens",
                summary_stats_among_admitted["total_admitted_wrong"])
    print_stats("Among tokens in wrong sentences, proportion admitted",
                summary_stats_among_admitted["admitted_proportion_wrong"])


def print_stats(label, list_of_values):
    if len(list_of_values) > 0:
        print(f"{label}: mean: {np.mean(list_of_values)}; std: {np.std(list_of_values)}; "
              f"min: {np.min(list_of_values)}: max: {np.max(list_of_values)}")
    else:
