"""
Calculate and graph quantiles to evaluate heuristics for uncertainty/prediction reliability. We illustrate that
the magnitude of the K-NN's output generally tracks that of the original model, and is at least as good a
sort comparator as the original model.
...
def generate_dual_line_graph(options, output_graph_file, sorted_quantiles, sorted_quantiles2):
    ...
    assert len(sorted_quantiles["knn_logit_magnitude"]) == len(sorted_quantiles2["original_model_logit_magnitude"])
    assert sorted_quantiles['n'] == sorted_quantiles2['n']
    assert len(sorted_quantiles['n']) == len(sorted_quantiles["knn_logit_magnitude"])
    quartile_as_percent_of_data = np.array(range(1, len(sorted_quantiles["knn_logit_magnitude"])+1)) / \
                                  len(sorted_quantiles["knn_logit_magnitude"])
    knn_color = 'tab:blue'
    original_model_color = "tab:green"
    n_color = 'tab:red'

    knn_line, = ax1.plot(sorted_quantiles['n'], sorted_quantiles['fscore_0_5'],
             's', color=knn_color, marker="o", markersize=2, linestyle="-", linewidth=1.0, label="K-NN")
    original_model_line, = ax1.plot(sorted_quantiles2['n'], sorted_quantiles2['fscore_0_5'],
             's', color=original_model_color, marker="+", markersize=2, linestyle="--", linewidth=1.0, label="Original")

    ax1.set_xlabel('Admitted n')
    ...
    # n_line, = ax2.plot(quartile_as_percent_of_data, sorted_quantiles['n'],
    #          's', color=n_color, marker="x", markersize=1, linestyle="-.", linewidth=1.0, label="Admitted n")  #linestyle='dashed', label="Admitted n")
    ...
    # # fig.tight_layout()
    #
    # #plt.legend((knn_line, original_model_line, n_line), ("K-NN", "Original", "Admitted n"), frameon=False, loc="lower right")
    ...
    if options.graph_type == 0:
        # plt.legend((knn_line, original_model_line), (r"+K$_8$NN\textsubscript{dist.}", "Original"), frameon=False,
        plt.legend((knn_line, original_model_line), (r"\textsc{uniCNN+BERT+K$_8$NN\textsubscript{dist.}}",
                                                     r"\textsc{uniCNN+BERT}"),
                   frameon=False,
                   loc=(0.01, 0.65))
        #plt.title(r'\textsc{uniCNN+BERT}')
...
def main():
    ...
    original_model_sorted_quantiles = \
        score_ranked_predictions(options, output_prediction_tuples, sort_key="original_model_logit_magnitude",
                                 tanh_of_prediction_logit=transform_logit,
                                 format_output_for_paper=options.format_output_for_paper)
    generate_dual_line_graph(options, options.output_graph_file, knn_sorted_quantiles, original_model_sorted_quantiles)
