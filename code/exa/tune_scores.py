# -*- coding: utf-8 -*-
"""
Tune (for sentence-level classification) the output from the binary cnn models. F1, F0.5, MCC
NOTE: This is an older debug script from an early version. This is queued for removal.
...
def get_labels_and_scores(filepath_with_name, print_output=False):
    ...
    with codecs.open(filepath_with_name, encoding="utf-8") as f:
        for line in f:
            ...
            logit = torch.tensor([float(line[2])])
...
def calculate_metrics(gold_labels, predicted_labels, print_output=False):
    ...
    acc = sum([1 if p == y else 0 for p, y in zip(predicted_labels, gold_labels)]) / len(predicted_labels)
    return acc
...
def main(arguments):
    ...
    # input_score_vals_file = input_score_vals_file_prefix
    ...
    for epoch in range(start_epoch, end_epoch+1):
        ...
        for offset in np.linspace(-1.0, 0.3, 150):
            #print("--" * 30)
            ...
            acc = calculate_metrics(gold_labels, predicted_labels)
            print(f"\tOffset: {offset}, acc: {acc}")
            ...
            max_acc, max_acc_offset, max_acc_epoch = update_stats(acc, max_acc, max_acc_offset, offset, epoch,
                                                                  max_acc_epoch)
    ...
    print(f"Max acc: {max_acc}; epoch {max_acc_epoch} offset {max_acc_offset}")
