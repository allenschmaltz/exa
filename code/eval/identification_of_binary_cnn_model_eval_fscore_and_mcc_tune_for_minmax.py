# -*- coding: utf-8 -*-
"""
Tune (for identification) the output from the binary cnn models. F1, F0.5, MCC. This assumes the input scores
for class 1 are the max logit across the document. The min values for class 0 are only included in the
scores file for reference and are zero'd below in get_labels_and_scores().
...
def get_labels_and_scores(filepath_with_name):
    ...
    with codecs.open(filepath_with_name, encoding="utf-8") as f:
        for line in f:
            ...
            logit = torch.tensor([0.0, float(line[2])])
...
def calculate_metrics(gold_labels, predicted_labels):
    ...
    accuracy = sum([1 if p == y else 0 for p, y in zip(predicted_labels, gold_labels)]) / len(predicted_labels)
...
def main(arguments):
    ...
    for offset in np.linspace(start_offset, end_offset, number_of_offsets):
        print("--" * 30)
